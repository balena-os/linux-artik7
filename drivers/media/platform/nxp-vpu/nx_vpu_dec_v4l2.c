...
int vpu_dec_parse_vid_cfg(struct nx_vpu_ctx *ctx)
{
	...
	if (ctx->strm_queue_cnt > 0) {
		...
		seqArg.seqDataSize = buf->vb.v4l2_planes[0].bytesused;
	...
	buf->vb.v4l2_buf.bytesused = ctx->strm_size;
	buf->vb.v4l2_buf.field = dec_ctx->interlace_flg[0];
	buf->vb.v4l2_buf.reserved = dec_ctx->frame_buf_delay;
...
int vpu_dec_decode_slice(struct nx_vpu_ctx *ctx)
{
	...
	if (dec_ctx->flush == 0) {
		...
		buf->used = 1;
		phyAddr = nx_vpu_mem_plane_addr(ctx, &buf->vb, 0);
		decArg.strmDataSize = buf->vb.v4l2_planes[0].bytesused;
		...
		decArg.eos = buf->vb.v4l2_buf.flags;
		timestamp.tv_sec = buf->vb.v4l2_buf.timestamp.tv_sec;
		timestamp.tv_usec = buf->vb.v4l2_buf.timestamp.tv_usec;
	...
	if (decArg.indexFrameDisplay >= 0) {
		...
		list_for_each_entry(buf, &dec_ctx->dpb_queue, list) {
			if (decArg.indexFrameDisplay
				...
				buf->vb.v4l2_buf.field = decArg.isInterace;
				buf->vb.v4l2_buf.flags = decArg.picType;
	...
	if (ctx->strm_queue_cnt > 0) {
		...
		buf->vb.v4l2_buf.index = idx;
		buf->vb.v4l2_buf.field = dec_ctx->interlace_flg[idx];
		buf->vb.v4l2_buf.bytesused = ctx->strm_size;
		buf->vb.v4l2_buf.reserved = dec_ctx->cur_reliable;
